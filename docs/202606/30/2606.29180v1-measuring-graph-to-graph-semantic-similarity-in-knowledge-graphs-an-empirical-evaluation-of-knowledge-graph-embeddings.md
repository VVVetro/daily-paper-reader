...
title_zh: 测量知识图谱中的图到图语义相似性：知识图谱嵌入的实证评估
...
tldr: 知识图谱中图级语义相似性度量尚未充分研究，现有方法依赖结构模式，无法保证语义对应。本文通过修改文本文档构建语义匹配KG对数据集，提出两种基于KG嵌入的评分函数：EmbPairSim和AvgEmbSim。实验表明EmbPairSim在MRR上比Sentence-BERT高5.3个百分点，且参数量更少。结果表明KG嵌入可作为图级语义相似性的有效信号。
source: arxiv
...
motivation: 现有KG嵌入方法缺乏图级语义度量，结构相似性不足以反映语义对应，需评估不同方法在图级语义相似性上的表现。
method: 通过修改文本文档并提取KG构建语义匹配数据集；提出EmbPairSim（最大成对实体相似性）和AvgEmbSim（频率加权质心）两种评分函数。
result: 在WikiText-2和CC-News上，EmbPairSim的MRR比Sentence-BERT高5.3个百分点，参数量显著更少。
conclusion: KG嵌入表示可作为紧凑有效的图级语义相似性信号，优于基于文本和结构的方法。
---

## 摘要
知识图谱以结构化三元组表示事实，广泛应用于组织跨领域的关系知识。正如文本信息从单词、句子到完整文档不等，知识图谱信息也可在多个层次上理解，包括实体、关系、三元组、子图乃至整个知识图谱。然而，现有的知识图谱嵌入方法主要关注实体、关系和三元组，图级语义在很大程度上未被解决。传统的图级方法通常基于结构模式比较图，但仅凭结构相似性无法保证知识图谱之间的语义相似性。为了评估不同方法捕捉此类图级语义信息的能力，我们研究了图到图语义相似性，即确定一对知识图谱是否表示语义对应的底层信息。为了获得可靠的 ground-truth 对应关系，我们通过修改文本文档、从原始和修改后的文档中提取知识图谱，并将其已知对应关系转移到知识图谱对，构建了一个语义匹配数据集。我们在每个数据集上比较了基于文本、基于结构和基于知识图谱嵌入的方法。对于基于知识图谱嵌入的方法，我们引入了两种评分函数：EmbPairSim（使用最大成对实体相似性）和 AvgEmbSim（使用频率加权质心）。在 WikiText-2 和 CC-News 上的实验表明，EmbPairSim 在参数显著更少的情况下，MRR 比 Sentence-BERT 高出最多 5.3 个百分点。这些结果表明，知识图谱嵌入表示可以作为知识图谱中图到图语义相似性的紧凑且有效的信号。我们的代码可在 https://github.com/SeungRyeolBaek/KG-to-KG-Semantic-Similarity 获取。
