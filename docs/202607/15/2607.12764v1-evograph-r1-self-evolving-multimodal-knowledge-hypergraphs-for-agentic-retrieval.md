...
tldr: 现有GraphRAG方法依赖静态知识图谱，无法适应迭代推理，导致跨模态碎片化、结构冻结、单次检索。EvoGraph-R1将检索建模为马尔可夫决策过程，通过查询、扩展、编辑、终止动作动态演化超图结构。在VQA和文本QA基准上，该方法显著提升了准确率、覆盖率和可追溯性。该工作建立了自演化知识图谱作为跨模态检索的通用范式。
source: arxiv
...
motivation: 静态知识图谱无法支持交互迭代推理，导致跨模态碎片化、结构冻结和单次检索的局限。
method: 将检索建模为马尔可夫决策过程，通过查询、扩展、编辑、终止动作自演化超图结构并整合新证据。
result: 在VQA和文本QA基准上，准确率、覆盖率和可追溯性显著优于现有RAG基线。
conclusion: 自演化知识图谱可作为跨模态检索的通用范式。
---

## 摘要
检索增强生成（RAG）已成为将多模态大语言模型（MLLM）锚定于外部知识的关键范式。近期图RAG方法引入了结构化实体关系图以改进检索与推理。然而，它们仍受限于将知识图谱视为离线构建、一次性查询的静态数据结构。这一静态范式与知识密集型推理的交互式、迭代特性相悖，造成三个瓶颈：（i）以文本为中心的分割阻碍了跨模态推理，（ii）冻结的结构无法纳入新证据或纠正错误，（iii）僵化的一次性检索缺乏自适应精炼。为克服这些局限，我们提出EvoGraph-R1，一种自演化的图RAG框架，将知识图谱重新构想为通过智能体交互塑造的动态环境。我们将检索形式化为马尔可夫决策过程（MDP），其中智能体观察图状态并执行动作——查询（GraphRetrieve）、扩展（WebSearch）、精炼（GraphEdit）或终止（Answer）推理。这些动作重塑超图结构并生成反馈信号，引导后续演化。通过这一闭环，超图通过整合新证据、纠正错误、精炼结构而不断演化，以支持多跳推理。在多模态VQA和文本QA基准上的实验表明，相较于现有RAG基线，该方法在准确性、覆盖率和可追溯性上均有显著提升，确立了自演化知识图谱作为跨模态基础范式的地位。
